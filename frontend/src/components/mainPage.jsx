...
import TimelineRift from "./timelineRift";
	...
	function createTimeLine(data) {
		...
		const timeLine = events.map(function (event) {
			if (event[0] == "obj" && event[1] == "dragon") {
				return (
					<TimelineDragon heartrate={-1} type={event[2]} time={event[4]} />
				);
			}
			if (event[0] == "obj" && event[1] == "baron") {
				return <TimelineBaron heartrate={-1} time={event[3]} />;
			}
			if (event[0] == "obj" && event[1] == "riftherald") {
				return <TimelineRift heartrate={-1} time={event[3]} />;
			}
			if (event[0] == "kill" && event[1] == focus) {
				return (
					<TimelineTakedown heartrate={-1} enemy={event[2]} time={event[4]} />
				);
			}
			if (event[0] == "kill" && event[2] == focus) {
				return (
					<TimelineTakendown heartrate={-1} enemy={event[1]} time={event[4]} />
				);
			}
		});
			...
			{timelineVisible && !loading ? (
				<>
					<Typography variant="h5">
						Displaying information for summoner: <strong>{val2}</strong> from{" "}
						<strong>{gamesBack2}</strong> games back
					</Typography>
